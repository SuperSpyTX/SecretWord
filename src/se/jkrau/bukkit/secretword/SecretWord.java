package se.jkrau.bukkit.secretword;
...
import se.jkrau.bukkit.secretword.Metrics.Graph;
import se.jkrau.bukkit.secretword.database.DatabaseCore;
import se.jkrau.bukkit.secretword.database.SecretPlayer;
import se.jkrau.bukkit.secretword.events.CoreEvents;
import se.jkrau.bukkit.secretword.events.EntityEvents;
import se.jkrau.bukkit.secretword.events.PlayerEvents;
...
public class SecretWord extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (arg1.equalsIgnoreCase("help") || arg1.equalsIgnoreCase("all")) {
			...
			if (Permissions.ADMIN_RELOAD.check(sender)) sender.sendMessage(Configuration.prefix + "/sw reload - Reload the configuration");
			
			if (Permissions.ADMIN_RESET.check(sender)) sender.sendMessage(Configuration.prefix + "/sw reset [player] - Reset a player's secret word (Relogin required for player).");
			...
			if (data != null) {
				...
				if (Configuration.notifyPlayerOfReset) sender.sendMessage(Configuration.prefix + ChatColor.GREEN + "The player was notified of the reset.");
			} else {
